...
<td style="width:50%; height: 0px; text-align: left;" colspan="4"><b><span style="border:1px solid black;">D.</span> Approved for Payment</b></td>
    ...
    <td style="width: 49%; height: 25px;text-align:center;font-size:14px;font-weight:bold;text-transform: uppercase;">'
        .'INGRID T. ABELLA-COLCOL'
        // .$this->getSignatory($model->osdv->osdv_id, 2, 'Osdv', 'DV','C', 65)['name']
        .'</td>
                                                                                                                         >
    ...
    <td style="width: 49%; height: 25px;font-size:13px;text-align:center;">'
        .'Accountant III'
        //.$this->getSignatory($model->osdv->osdv_id, 2, 'Osdv', 'DV','C', 65)['position']
        .'</td>
...
<td style="width: 16.67%; height: 50px; text-align: right;padding:5px;vertical-align:top; font-weight: bold;">';
        ...
        for($i=0; $i<count($keys); $i++){
            $account = Accounttransaction::findOne($keys[$i]);
            if($account){
                if($account->debitcreditflag === 1)
                    $text .= number_format($model->amount, 2).'<br/>';
                else
                    $text .= '-';
            }
